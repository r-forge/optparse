...
By default \emph{optparse} will generate a help message if it encounters \verb|--help| or \verb|-h| on the command line.  Note how \verb|%default| in the example program was replaced by the actual default values by \emph{optparse}.  
\newline\newline
\noindent\verb|\Sexpr{paste("bash$", command)}|
<<echo=FALSE>>=
...
<<echo=FALSE>>=
cat(system(sprintf("%s 2>&1", command),  intern=TRUE), sep="\n")   
command <- "Rscript example.R --mean=10 --sd=10 --count=3" 
@
...
<<echo=FALSE>>=
cat(system(sprintf("%s 2>&1", command),  intern=TRUE), sep="\n")   
command <- "Rscript example.R --quiet -c 4 --generator=\"runif\"" #  same as above but "quiet"
@
\end{samepage}
...
<<echo=FALSE>>=
cat(system(sprintf("%s 2>&1", command),  intern=TRUE), sep="\n")   
command <- "Rscript example.R --silent -m 5" #  same as above but "quiet"
@
