...
show_file_command <- "Rscript display_file.Rscript example.Rscript"
chmod_command <- "chmod u+g example.Rscript"
@

In our working directory we have two example R scripts, named ``example.Rscript'' and ``display\_file.Rscript'' illustrating the use of the optparse package.
\newline\newline
...
\newline\newline
\noindent\verb|\Sexpr{paste("bash$", show_file_command)}|
<<echo=FALSE>>=
cat(system(sprintf("%s", show_file_command), intern=TRUE), sep="\n")
@

In order for a *nix system to recognize the ``\#!''-shebang line you need to mark the file executable with the ``chmod'' command:
\newline\newline
\noindent\verb|\Sexpr{paste("bash$", chmod_command)}|

\newpage
<<echo=FALSE>>=
command <- "example.Rscript --help" # same as system("Rscript example.Rscript -h")
@
...
command <- "example.Rscript" # rely only on defaults
@
...
<<echo=FALSE>>=
cat(system(sprintf("%s %s 2>&1", Rscript_executable, command),  intern=TRUE), sep="\n")   
command <- "example.Rscript --mean=10 --sd=10 --count=3" 
@
...
<<echo=FALSE>>=
cat(system(sprintf("%s %s 2>&1", Rscript_executable, command),  intern=TRUE), sep="\n")   
command <- "example.Rscript --quiet -c 4 --generator=\"runif\"" #  same as above but "quiet"
@
\end{samepage}
...
<<echo=FALSE>>=
cat(system(sprintf("%s %s 2>&1", Rscript_executable, command),  intern=TRUE), sep="\n")   
command <- "example.Rscript --silent -m 5" #  same as above but "quiet"
@
...
<<echo=FALSE>>=
cat(system(sprintf("%s %s 2>&1", Rscript_executable, command),  intern=TRUE), sep="\n")   
command <- "example.Rscript -c 100 -c 2 -c 1000 -c 7" #  same as above but "quiet"
@
...
Currently, \emph{optparse} does not recognize positional arguments, one needs to pull them out first.  Below we give an example program \emph{display\_file.Rscript}, which is a program that prints out the contents of a single file (which is a required argument, not an optional argument) and which accepts the normal help option as well as an option to add line numbers to the output.
\newpage
<<echo=FALSE>>=
command <- "display_file.Rscript --help" 
@
\noindent\verb|\Sexpr{paste("bash$", command)}|
<<echo=FALSE>>=
cat(system(sprintf("%s %s 2>&1", Rscript_executable, command),  intern=TRUE), sep="\n")   
command <- "display_file.Rscript --add_numbers display_file.Rscript" 
@
\noindent\verb|\Sexpr{paste("bash$", command)}|
<<echo=FALSE>>=
cat(system(sprintf("%s %s 2>&1", Rscript_executable, command),  intern=TRUE), sep="\n")   
