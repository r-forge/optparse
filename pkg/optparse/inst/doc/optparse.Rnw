...
chmod_command <- "chmod ug+x display_file.Rscript example.Rscript"
...
command <- "display_file.Rscript example.Rscript" # to show file
@
...
<<echo=FALSE>>=
cat(system(sprintf("%s %s 2>&1", Rscript_executable, command),  intern=TRUE), sep="\n")   
command <- "display_file.Rscript non_existent_file.txt" 
@
\noindent\verb|\Sexpr{paste("bash$", command)}|
<<echo=FALSE>>=
cat(system(sprintf("%s %s 2>&1", Rscript_executable, command),  intern=TRUE), sep="\n")   
command <- "display_file.Rscript"
@
\newpage
\noindent\verb|\Sexpr{paste("bash$", command)}|
<<echo=FALSE>>=
