...
show_file_command <- "cat example.Rscript"
...
In our working directory we have an example R script, named ``example.Rscript'' illustrating the use of the optparse package.
\newline\newline
...
If you specify an illegal flag then \emph{optparse} will throw an error.  \emph{optparse} will also throw an error if you specify two options that use identical flags. 
\newline\newline
\noindent\verb|\Sexpr{paste("bash$", command)}|
<<echo=FALSE>>=
cat(system(sprintf("%s %s 2>&1", Rscript_executable, command),  intern=TRUE), sep="\n")   
